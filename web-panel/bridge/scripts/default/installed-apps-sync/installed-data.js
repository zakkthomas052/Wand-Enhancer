        ...
        return {
            ...
            catalog: isRecord(state.installedAppsService.catalog)
                ...
                : isRecord(storeState?.catalog)
                  ? storeState.catalog
                  : {},
            installedGameVersions: isRecord(state.installedAppsService.installedVersions)
...
export async function refreshUnavailableTitles(state, rawInstalledApps, force = false) {
    ...
    state.unavailableTitlesFetchPromise = fetchUnavailableTitles(state, correlationIds);
    await state.unavailableTitlesFetchPromise;
    return state.unavailableTitlesById;
}
...
export function buildSnapshot(state) {
    ...
    for (const [gameId, versions] of Object.entries(installedGameVersions)) {
        ...
        const preferredApp = pickPreferredInstalledApp(rawInstalledApps, game, versions);
        if (!preferredApp) {
            continue;
        }
    ...
    for (const unavailableTitle of Object.values(correlatedUnavailableTitles)) {
        ...
        for (const game of unavailableTitle.games) {
            ...
            const preferredApp = pickPreferredInstalledApp(rawInstalledApps, game);
            if (!preferredApp) {
                continue;
            }
    ...
    return {
        ...
        diagnostics: {
            ...
            correlatedUnavailableTitles: Object.keys(correlatedUnavailableTitles).length,
            matchedCatalogGames,
...
export function toInstalledAppRecord(correlationId, app) {
    if (!isRecord(app) || typeof correlationId !== 'string' || !correlationId.trim()) {
        return null;
    }
    ...
    return {
        ...
        imageUrl: pickImageUrl(
            ...
            app.images,
            app.assets,
            getSteamClientIconUrl(findSteamAppId(app), getInstalledAppSteamAppId(platform, sku)),
        ),
...
export function compareInstalledAppRecords(left, right) {
    const lastPlayedDiff =
        (right.platformLastPlayedTimestamp ?? 0) - (left.platformLastPlayedTimestamp ?? 0);
    if (lastPlayedDiff !== 0) {
        return lastPlayedDiff;
    }

    const playtimeDiff =
        (right.platformTotalPlaytimeMinutes ?? 0) - (left.platformTotalPlaytimeMinutes ?? 0);
    if (playtimeDiff !== 0) {
        return playtimeDiff;
    }
...
function getStoreState(storeRef) {
    const state =
        typeof storeRef?.state?.getValue === 'function' ? storeRef.state.getValue() : null;
...
async function fetchUnavailableTitles(state, correlationIds) {
    ...
    try {
        for (let index = 0; index < correlationIds.length; index += UNAVAILABLE_TITLES_BATCH_SIZE) {
            const batch = correlationIds.slice(index, index + UNAVAILABLE_TITLES_BATCH_SIZE);
            const response =
                await state.unavailableTitlesService.getUnavailableTitlesByCorrelationIds(batch);
        ...
        state.log('warn', 'Unavailable titles refresh failed.', formatError(error));
    } finally {
...
function normalizeUnavailableTitlesResponse(value) {
    const titles = Array.isArray(value) ? value : Array.isArray(value?.data) ? value.data : [];
...
function collectCorrelationIds(game, versions) {
    ...
    if (Array.isArray(versions)) {
        for (const version of versions) {
            if (typeof version?.correlationId === 'string' && version.correlationId.trim()) {
                entries.push({
...
export function rankInstalledAppCandidates(rawInstalledApps, game, versions) {
    return Array.from(
        new Map(
            collectCorrelationIds(game, versions).map((entry) => [entry.correlationId, entry]),
        ).values(),
    )
        .map((entry) => {
