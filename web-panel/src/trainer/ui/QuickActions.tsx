...
const Chip = ({ icon, label, variant, onClick }: ChipProps) => {
    return (
        <button
            type="button"
            className={cn(
                'inline-flex shrink-0 items-center gap-1.5 rounded-full border px-3 py-1.5 text-[11.5px] font-semibold backdrop-blur-xl',
                getChipVariantClass(variant),
            )}
            onClick={onClick}
        >
...
const PresetChip = ({ preset, onApply, onDelete }: PresetChipProps) => {
    ...
    return (
        <span className="inline-flex shrink-0 overflow-hidden rounded-full border border-white/10 bg-white/5.5 text-[11.5px] font-semibold text-(--deck-fg-2) shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] backdrop-blur-xl">
            <button
                type="button"
                className="inline-flex h-7.5 max-w-35 items-center gap-1.5 px-3"
                onClick={handleApply}
            >
            ...
            <button
                type="button"
                aria-label={_(msg`Delete preset ${preset.name}`)}
                className="flex h-7.5 w-7 items-center justify-center border-l border-white/10 text-(--deck-fg-4) hover:text-(--deck-fg)"
...
const PresetModal = ({ draftName, onClose, onDraftNameChange, onSubmit }: PresetModalProps) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/45 p-3 backdrop-blur-[6px] sm:items-center">
            <button
                type="button"
                aria-label={_(msg`Close preset modal`)}
                className="absolute inset-0"
                onClick={onClose}
            />
            <form
                className="remote-glass-drawer relative w-full max-w-90.5 rounded-[18px] border border-white/10 p-4"
            ...
            >
                <div className="mb-3 flex items-start justify-between gap-3">
                    ...
                    <IconButton
                        shrink
                        label={_(msg`Close preset modal`)}
                        icon="x"
                        onClick={onClose}
                    />
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                    <button
                        type="button"
                        className="remote-glass-control h-10 rounded-[10px] border text-[12px] font-semibold text-(--deck-fg-2)"
                        onClick={onClose}
                    >
                        <Trans>Cancel</Trans>
                    </button>
                    <button
                        type="submit"
                        className="h-10 rounded-[10px] bg-(--deck-accent) text-[12px] font-bold text-black shadow-[0_8px_24px_-8px_var(--deck-accent)]"
                    >
                        <Trans>Save</Trans>
                    </button>
                </div>
...
function getChipVariantClass(variant: ChipProps['variant']): string {
    ...
    return 'border-white/10 bg-white/5.5 text-(--deck-fg-2) shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]';
}
