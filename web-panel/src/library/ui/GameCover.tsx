...
export const GameCover = ({ game, size = 'sm' }: GameCoverProps) => {
    ...
    return (
        <div
            className={`relative shrink-0 overflow-hidden border border-white/6 bg-[linear-gradient(135deg,rgba(238,0,255,0.28),rgba(0,0,0,0.48))] shadow-[inset_0_0_0_1px_rgba(255,255,255,.04),0_2px_6px_rgba(0,0,0,.35)] ${sizeClass}`}
        >
            {imageUrl ? (
                <img
                    alt=""
            ...
            {game.running ? (
                <span className="absolute right-1 top-1 size-1.5 rounded-full bg-(--deck-accent) shadow-[0_0_6px_var(--deck-accent)]" />
            ) : null}
